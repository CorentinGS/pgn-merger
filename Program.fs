    ...
    if args.Length = 0 then
        ...
        let rec loop i opts =
            if i >= args.Length then
                ...
                | "--verbose" ->
                    loop (i + 1) { opts with Verbose = true }
                | "--quiet" ->
                    loop (i + 1) { opts with Quiet = true }
                | "--help" | "-h" | "-?" ->
                    None
                | arg when arg.StartsWith("--") ->
                    ...
                    None
                | _ when i = 0 ->
                    loop (i + 1) { opts with FolderPath = args.[0] }
                | _ ->
...
let findPgnFiles folderPath recursive =
    let searchOption =
        if recursive then SearchOption.AllDirectories
...
let formatThroughput (bytes: int64) (elapsed: TimeSpan) =
    ...
    if seconds > 0.0 then
        ...
        if bytesPerSec > 1048576.0 then
            sprintf "%.2f MB/s" (bytesPerSec / 1048576.0)
        elif bytesPerSec > 1024.0 then
            sprintf "%.2f KB/s" (bytesPerSec / 1024.0)
        else
            sprintf "%.0f B/s" bytesPerSec
    else
        "N/A"
...
let main argv =
    ...
    | Some opts ->
        ...
        if not (Directory.Exists folderPath) then
            printfn "Error: Directory '%s' does not exist" folderPath
            1
        else
            try
                ...
                if pgnFiles.Length = 0 then
                    ...
                    if not opts.Quiet then
                        printfn "Found %d PGN files to merge" pgnFiles.Length

                    let totalBytes =
                        pgnFiles
                        |> Array.map FileInfo
                        |> Array.sumBy (fun fi -> fi.Length)
                    ...
                    use outputStream = new FileStream(outputPath, FileMode.Create, FileAccess.Write, FileShare.None, 65536)
                    ...
                    for pgnFile in pgnFiles do
                        try
                            ...
                            if opts.Verbose && not opts.Quiet then
                                ...
                                printfn "[%5.1f%%] %s (%s) - %s"
                                    percent fileName (formatBytes fileSize) throughput
                        ...
                        | :? IOException as ex ->
                            ...
                            printfn "Error: IO error processing '%s': %s" pgnFile ex.Message
                        | ex ->
                            results <- Failure (pgnFile, ex.Message) :: results
                            printfn "Error: Unexpected error processing '%s': %s" pgnFile ex.Message

                    writer.Flush()
                    stopwatch.Stop()

                    let successCount = results |> List.filter (function Success _ -> true | _ -> false) |> List.length
                    let failureCount = results.Length - successCount

                    if not opts.Quiet then
                        printfn ""
                        printfn "Successfully merged %d of %d PGN files into: %s" successCount pgnFiles.Length outputPath
